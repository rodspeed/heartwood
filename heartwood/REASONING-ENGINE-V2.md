...
│  cerebro_search · cerebro_context · cerebro_propose · cerebro_read│
└────────────────────────────┬────────────────────────────────────┘
                             │
┌────────────────────────────▼────────────────────────────────────┐
...
```python
@mcp.tool()
def cerebro_search(query: str, top_k: int = 10) -> list[dict]:
...
@mcp.tool()
def cerebro_read(note_id: str) -> dict:
...
@mcp.tool()
def cerebro_propose(title: str, body: str, tags: list[str],
...
@mcp.tool()
def cerebro_context(topic: str) -> dict:
...
- `cerebro_beliefs` — needs Phase 4 (belief layer)
- `cerebro_reflect` — needs Phase 3 (memory system with reflection nodes)
- `cerebro_neighbors` — nice-to-have, not required for launch

### 6.2 Inbox System

The `cerebro_propose` tool writes to an inbox, not directly to the graph:
...
The `cerebro_context` tool is the most valuable new capability. It returns a single structured object that gives any agent your full context on a topic:
...
@server.tool()
async def cerebro_search(query: str, top_k: int = 10) -> list[dict]:
...
| Failure | Behavior |
|---------|----------|
| Embeddings not loaded | `cerebro_search` falls back to full-text only; `cerebro_context` returns notes without relevance scores |
| Note not found | `cerebro_read` returns `{error: "not found"}`, not an exception |
| Inbox write fails | `cerebro_propose` returns `{error: "..."}` with the error; no partial state |
...
- **Propose → review loop:** Create 10 inbox items via `cerebro_propose`, review them in the UI. Is the triage experience fast enough to be practical?
...
```python
@mcp.tool()
def cerebro_reflect(query: str) -> dict:
...
- **MCP tool:** `cerebro_beliefs` (tool #6) — semantic search across claims, filter by note/confidence.
...
```python
@mcp.tool()
def cerebro_beliefs(topic: str) -> list[dict]:
...
```python
@mcp.tool()
def cerebro_neighbors(note_id: str, hops: int = 2) -> list[dict]:
